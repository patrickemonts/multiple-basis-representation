...
from tfimsim.hamiltonian import EDSimulatorConfig,EDSimulatorQuimb,PEPSSimulator,PEPSSimulatorConfig, MPSSimulatorConfig, MPSSimulator # Probably something to change here, but this is Patrick's knowledge
...
def args2logname(args):
    ...
    if args.nsteps is not None:
        fname = f"log_cluster_L_{args.nx:02d}-{args.ny:02d}_J_{args.J:0.2f}_hmin_{args.hmin:.2f}_hmax_{args.hmax:.2f}_nsteps_{args.nsteps:03d}_type_{args.type}{args2suffix(args)}.log"
    else:
        fname = f"log_cluster_L_{args.nx:02d}-{args.ny:02d}_J_{args.J:0.2f}_h_{args.hmin:.2f}_type_{args.type}{args2suffix(args)}.log"
...
def args2fname(args):
    ...
    """
    if args.nsteps is not None:
        fname = f"data_cluster_L_{args.nx:02d}-{args.ny:02d}_J_{args.J:0.2f}_hmin_{args.hmin:.2f}_hmax_{args.hmax:.2f}_nsteps_{args.nsteps:03d}_type_{args.type}{args2suffix(args)}.csv"
    else:
        fname = f"data_cluster_L_{args.nx:02d}-{args.ny:02d}_J_{args.J:0.2f}_h_{args.hmin:.2f}_type_{args.type}{args2suffix(args)}.csv"
    return fname
...
def main(args):
    ...
    if not os.path.exists(path_out) or args.overwrite:
        ...
        logging.info(f"Problem: cluster")
        logging.info("================================")
