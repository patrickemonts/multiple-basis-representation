...
def evaluate_all_energies_xx(bitstrings_x, bitstrings_z, edges):
    ...
    for i, xi in enumerate(bitstrings_x):
        for e in edges:
            if xi[e[0]] == xi[e[1]]: energies_xx[i, i] -= 1
            else: energies_xx[i, i] += 1
    ...
    energies = np.block([[energies_xx, energies_xz], [np.conj(energies_xz.T), energies_zz]])
...
def evaluate_all_energies_z(bitstrings_x, bitstrings_z):
    ...
    for i, xi in enumerate(bitstrings_x): # XX terms
        for j, xj in enumerate(bitstrings_x):
            xor = _xor(xi, xj)
            if xor.count('1') == 1: energies_xx[i, j] -= 1
...
def compute_overlap_matrix(bitstrings_x, bitstrings_z):  # Computation of the overlap matrix, for sets of bitstrings in the computational and Hadamard bases
    f = np.ones((len(bitstrings_x), len(bitstrings_z))) / (2**(.5 * len(bitstrings_x[0])))
...
def generate_bitstrings(n, k, mode='0'):
    ...
    for indices in index_combinations:
        if mode == '0': bitstring = ['0'] * n  # Initialize with all 0s
        elif mode == '1': bitstring = ['1'] * n  # Initialize with all 1s
        for index in indices:
            if mode == '0': bitstring[index] = '1'  # Set 1s at specified indices
            if mode == '1': bitstring[index] = '0'  # Set 1s at specified indices
        bitstrings.append(''.join(bitstring))  # Join list to form a string and append to the result list
...
def create_z_list(nqubits, degree): # List of all bitstrings in the computational bases that we consider
    bitstrings = []
    for k in range(degree+1): 
        bitstrings += generate_bitstrings(nqubits, k, mode='0')
...
def create_edges(nx, ny): # Function to create the edges of a square lattice
        ...
        for i in range(nx - 1):
            for j in range(ny): 
                edges.append((i + nx * j, i + 1 + nx * j)) # Z(i + nx * j) * Z(i + 1 + nx * j)
        for i in range(nx):
            for j in range(ny-1): 
                edges.append((i + nx * j,i + nx * (j + 1))) # Z(i + nx * j) * Z(i + nx * (j + 1))
        return edges