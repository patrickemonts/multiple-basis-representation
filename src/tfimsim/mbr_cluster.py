# DIAGONALIZATION OF A Cluster state parent hamiltonian AS GIVEN BY A GRAPH

import numpy as np
from itertools import combinations

# in this file, x refers to the cluster state

def _evaluate_energies_cluster(bitstrings_x, bitstrings_z, edges): 
    energies_zz = np.zeros((bitstrings_z.shape[0],)*2)

    for q in range(bitstrings_z.shape[1]):
        energ = np.zeros((bitstrings_z.shape[0],)*2)
        ...
        for i, zi in enumerate(bitstrings_z): 
            for j, zj in enumerate(bitstrings_z_): 
                energ[i, j] -= int((zi == zj).all())
        add2 = np.zeros(bitstrings_z.shape[1], dtype = int)
        for e in edges:
            if q in e:
                e1, e2 = e
                if e1 == q:
        ...
        signs = (-1) ** (bitstrings_z @ np.vstack([add2]*len(bitstrings_z)).transpose())
        energ = signs * energ
        energies_zz += energ
...
def _evaluate_energies_z(bitstrings_x, bitstrings_z, edges):
    energies_zz = np.sum(2 * bitstrings_z - 1, axis=1)
    return np.diag(energies_zz)

def _evaluate_energies_crossed(bitstrings_x, bitstrings_z, edges): 
...
def evaluate_all_energies_cluster(bitstrings_x, bitstrings_z, edges):
    # Function to evaluate all energies given by the $XX$ hamiltonian, specified by bitstring in the computational and hadamard bases
    # Extension of evaluate_energies_xx
